...
interface PickedTool {
  id: string; fabIcon: ReactNode; fabLabel: string; action?: () => void; fabColor?: string;
}
  ...
  for (const key of pageKeys) {
    ...
    for (const t of tools) {
      ...
      if (t.action || t.primary) {
        result.push({ id: t.id, fabIcon: t.fabIcon, fabLabel: t.fabLabel || t.id, action: t.action || (() => {}), fabColor: t.fabColor });
      }
    }
  }
  return result;
...
export default function PageSpeedDial({ pageKeys, searchTool }: PageSpeedDialProps) {
      ...
      >
        ...
        {tools.map(tool => (
          <SpeedDialAction
            ...
            icon={tool.fabIcon}
            title={tool.fabLabel}
            slotProps={tool.fabColor ? { fab: { sx: { bgcolor: `${tool.fabColor}.main`, '&:hover': { bgcolor: `${tool.fabColor}.dark` } } } } : undefined}
            onPointerDown={e => { if (e.button !== 0) return; e.preventDefault(); e.stopPropagation(); tool.action?.(); setOpen(false); }}
          />
        ))}
      </SpeedDial>
      {searchTool && (
