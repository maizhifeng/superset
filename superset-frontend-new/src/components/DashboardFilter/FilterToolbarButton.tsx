...
export default function FilterToolbarButton({
  activeCount, hiddenFilters, onOpenDrawer, onClearAll, onAddFilter,
  ...
  return (
    <>
      <Button
        ...
        startIcon={
          <Badge badgeContent={activeCount} color="primary" overlap="circular" slotProps={{ badge: { sx: { height: 18, minWidth: 18, fontSize: 11 } } }}>
      ...
      {(activeCount > 0 || hiddenFilters.length > 0) && (
        <IconButton onClick={e => setAnchorEl(e.currentTarget)} sx={{ p: 0.125, ml: -0.25 }}>
      ...
      >
        <MenuItem onClick={() => { setAnchorEl(null); onOpenDrawer(); }} dense>
          <ListItemIcon><FilterListIcon fontSize="small" /></ListItemIcon>
          Filter panel
        </MenuItem>
        {activeCount > 0 && (
          <MenuItem onClick={() => { setAnchorEl(null); onClearAll(); }} dense>
        ...
        {hiddenFilters.slice(0, 20).map(f => (
          <MenuItem key={f.id} onClick={() => { setAnchorEl(null); onAddFilter(f.id); }} dense sx={{ fontSize: '0.8125rem' }}>
