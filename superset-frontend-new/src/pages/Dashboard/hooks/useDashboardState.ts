...
export function useDashboardState() {
  ...
  const compare = useDashboardCompare({ chartMeta, chartData, chartDataRef, buildAdhocFiltersRef });
  ...
  const dashboardChartIds = useMemo(() => {
    ...
    for (const n of Object.values(nodeMap)) {
      if (n.type === "CHART" && n.meta?.chartId) ids.add(Number(n.meta.chartId));
    }
    return ids;
  }, [nodeMap]);

  const layout = useDashboardLayout({ dashboardId: id ?? "", nodeMap, chartMeta, onNodeMapChange: setNodeMap });
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      setDashboard(dash);
      if (prevTitleRef.current !== dash.dashboard_title) prevTitleRef.current = dash.dashboard_title;
      ...
      const chartIds = Object.values(parsedNodes).filter((n) => n.type === "CHART").map((n) => n.meta?.chartId as number).filter(Boolean);
      ...
      if (chartIds.length > 0) {
        ...
        for (let i = 0; i < chartIds.length; i += CONCURRENCY) {
          ...
          const results = await Promise.allSettled(batch.map((cid) => api.get<{ result: ChartData }>(`/chart/${cid}`).then((r) => r.data.result)));
          results.forEach((r) => { if (r.status === "fulfilled" && r.value) metaMap[r.value.id] = r.value; });
        }

        const dsIds = new Set<number>();
        for (const chart of Object.values(metaMap)) { const dsId = chart.datasource_id || 0; if (dsId) dsIds.add(dsId); }
        ...
        if (dsIds.size > 0) {
          ...
          for (const dsId of dsIds) {
            try {
              const dataset = await getDataset<{ columns: { column_name: string; type: string | null; is_dttm: boolean; extra: string | null }[] }>(dsId);
              ...
              for (const col of cols) {
                ...
                try { extra = col.extra ? JSON.parse(col.extra) : null; } catch { /* ignore */ }
                if (extra?.dashboard_filter !== true) continue;
                if (col.is_dttm) timeCols.push({ datasetId: dsId, column: col.column_name, name: col.column_name, columnType: "time" });
                else if (stringTypes.test(col.type)) stringCols.push({ datasetId: dsId, column: col.column_name, name: col.column_name, columnType: "string" });
        ...
        for (const dsId of dsIds) { try { fmtMaps[dsId] = await getMetricFormatMap(dsId); } catch { /* ignore */ } }
        setMetricFormatMaps(fmtMaps);
        await new Promise((resolve) => setTimeout(resolve, 20));
        const { dataMap, totalRowMap } = await getChartDataWithFilters(chartIds, metaMap, buildAdhocFiltersRef.current, false);
  ...
  const cycleInterval = useCallback(() => {
    if (intervalSeconds === 0) { setIntervalSeconds(PRESET_INTERVALS[0]); notify({ severity: "success", message: `自动刷新已切换至 ${PRESET_INTERVALS[0] >= 60 ? `${PRESET_INTERVALS[0] / 60}分钟` : `${PRESET_INTERVALS[0]}秒`}` }); return; }
  ...
  const refreshChart = useCallback(async (chartId: number, page?: number) => {
    setChartLoading((prev) => ({ ...prev, [chartId]: true }));
    try {
      const result = await refreshChartData(chartId, chartMetaRef.current, buildAdhocFiltersRef.current, page);
      if (result) {
        setChartData((prev) => ({ ...prev, [chartId]: result.data }));
        if (result.hasMore !== undefined) setChartHasMore((prev) => ({ ...prev, [chartId]: result.hasMore as boolean }));
        const cc = compareConfigRef.current;
        if (cc?.enabled && cc.chartId === chartId) fetchMirrorRef.current(chartId, cc.dimensions, result.data);
      }
    } finally { setChartLoading((prev) => ({ ...prev, [chartId]: false })); }
  }, [refreshChartData]);

  const handleChartPageChange = useCallback((chartId: number, newPage: number) => {
    setChartPages((prev) => ({ ...prev, [chartId]: newPage }));
    setChartLoading((prev) => ({ ...prev, [chartId]: true }));
    refreshChart(chartId, newPage);
  }, [refreshChart]);

  const refreshCharts = useCallback(async (chartIds?: Set<number>) => {
    refreshFilterValues();
    const meta = chartMetaRef.current;
    const ids = chartIds ? Array.from(chartIds) : Object.keys(meta).map(Number);
    if (ids.length === 0) return;
    setChartLoading((prev: Record<number, boolean>) => { const next: Record<number, boolean> = { ...prev }; for (const id of ids) next[id] = true; return next; });
    setChartPages((prev) => { const next = { ...prev }; for (const id of ids) delete next[id]; return next; });
    setChartHasMore((prev: Record<number, boolean>) => Object.fromEntries(Object.entries(prev).filter(([k]) => !ids.includes(Number(k)))) as Record<number, boolean>);
    try {
      const { dataMap, totalRowMap } = await getChartDataWithFilters(ids, meta, undefined, true);
      setChartData((prev) => ({ ...prev, ...dataMap }));
      setTotalRows((prev) => ({ ...prev, ...totalRowMap }));
      const cc = compareConfigRef.current;
      if (cc?.enabled) { const freshData = dataMap[cc.chartId]; fetchMirrorRef.current(cc.chartId, cc.dimensions, freshData); }
    } finally { setChartLoading((prev) => { const next = { ...prev }; for (const id of ids) next[id] = false; return next; }); }
  }, [getChartDataWithFilters]);
  ...
  useDashboardToolbar({
    dashboard, activeCount, hiddenFilters, clearAll, layoutItems,
    onFilterDrawerOpen: () => setFilterDrawerOpen(true),
    onAddFilter: (id: string) => setPendingFilterIds((prev) => [...prev, id]),
    onRefreshAll: refreshCharts, onOpenNav: openNav,
  ...
  const handleCloseDrawer = useCallback(() => { setSearchParams((prev) => { prev.delete("slice_id"); return prev; }); }, [setSearchParams]);
  ...
  const handleFilterChange = useCallback((filterId: string, value: unknown) => {
    ...
    const affectedIds = changedFilter?.chartsInScope?.length ? new Set(changedFilter.chartsInScope) : undefined;
    setChartLoading((prev) => { const next = { ...prev }; const ids = affectedIds ? Array.from(affectedIds) : Object.keys(chartMetaRef.current).map(Number); for (const cid of ids) next[cid] = true; return next; });
    filterTimerRef.current = setTimeout(() => refreshChartsRef.current(affectedIds), 300);
  }, [setFilter]);
  ...
  const handleOpenInsight = useCallback((chartId: number) => {
    ...
    for (const f of filters) {
      ...
      if (state?.value != null) activeFilters[f.id] = { value: state.value, column: f.column, filterType: f.filterType };
    }
    openAiDrawer("insight", { chartId, chartMeta: chartMeta[chartId], filters: activeFilters, dashboardId: id });
  }, [filters, filterState, chartMeta, id]);

  const handleAddChartSelect = useCallback(async (chart: { id: number; slice_name: string; viz_type: string }) => {
    setAddChartDialogOpen(false);
    const alreadyInLayout = Object.values(layout.nodeMapRef.current).some((n) => n.type === "CHART" && n.meta?.chartId === chart.id);
    if (alreadyInLayout) return;
    let chartMetaData: ChartData | undefined;
    try { const metaRes = await api.get<{ result: ChartData }>(`/chart/${chart.id}`); chartMetaData = metaRes.data?.result; } catch { /* ignore */ }
    if (chartMetaData) { chartMetaRef.current = { ...chartMetaRef.current, [chart.id]: chartMetaData }; setChartMeta((prev) => ({ ...prev, [chart.id]: chartMetaData })); }
    const chartKey = `CHART-${chart.id}`;
    const newNode: LayoutNode = { id: chartKey, type: "CHART", children: [], meta: { chartId: chart.id, width: 4, height: 30, sliceName: chart.slice_name } };
    let gridNode = Object.values(layout.nodeMapRef.current).find((n) => n.type === "GRID");
    let rootNode = Object.values(layout.nodeMapRef.current).find((n) => n.type === "ROOT");
    const updatedNodeMap = { ...layout.nodeMapRef.current, [chartKey]: newNode };
    if (!gridNode || !rootNode) {
      const grid: LayoutNode = { id: "GRID_ID", type: "GRID", children: [chartKey] };
      const root: LayoutNode = { id: "ROOT_ID", type: "ROOT", children: ["GRID_ID"] };
      updatedNodeMap["ROOT_ID"] = root; updatedNodeMap["GRID_ID"] = grid; setGridId("GRID_ID");
    } else {
      updatedNodeMap[gridNode.id] = { ...gridNode, children: [...(gridNode.children || []), chartKey] };
    }
    layout.nodeMapRef.current = updatedNodeMap;
    setNodeMap(updatedNodeMap);
    layout.fullPositionRef.current = { ...layout.fullPositionRef.current, ...updatedNodeMap } as DashboardPosition;
    await layout.saveLayout();
    if (chartMetaData) {
      try { const { dataMap, totalRowMap } = await getChartDataWithFilters([chart.id], { [chart.id]: chartMetaData }); setChartData((prev) => ({ ...prev, ...dataMap })); setTotalRows((prev) => ({ ...prev, ...totalRowMap })); } catch { /* ignore */ }
    }
  }, [layout, getChartDataWithFilters]);

  const handleDeleteChart = useCallback(async (chartId: number) => {
    ...
    for (const [key, n] of Object.entries(layout.nodeMapRef.current)) {
      ...
      if (n.children?.includes(chartKey)) updatedNodeMap[key] = { ...n, children: n.children.filter((c) => c !== chartKey) };
      else updatedNodeMap[key] = n;
    }
    delete updatedNodeMap[chartKey];
    layout.nodeMapRef.current = updatedNodeMap;
    setNodeMap(updatedNodeMap);
    setChartMeta((prev) => { const next = { ...prev }; delete next[chartId]; return next; });
    const metaRef = chartMetaRef.current;
    if (metaRef[chartId]) { const next = { ...metaRef }; delete next[chartId]; chartMetaRef.current = next; }
    setChartData((prev) => { const next = { ...prev }; delete next[chartId]; return next; });
    setDashboardDimensions([]);
    await layout.saveLayout();
  }, [layout]);

  useEffect(() => { if (!filterDrawerOpen) setPendingFilterIds([]); }, [filterDrawerOpen]);
  ...
  return {
    ...
    handleFilterChange, handleClearAll, handleChartPageChange, refreshChart, refreshCharts, cycleInterval,
