...
import { FilterToolbarButton, useDashboardFilters } from '@/components/DashboardFilter';
...
export default function DashboardToolbar({
  dashboard, nodeMap, gridId, chartMeta, refreshAllCharts,
  onFilterDrawerToggle, onPendingFilterIds,
  ...
  const { filters, filterState: _filterState, clearAll, buildAdhocFilters: _buildAdhocFilters, activeCount } =
    useDashboardFilters(dashboard?.json_metadata ?? null, []);

  const hiddenFilters = filters.slice(8);

  const layoutItems = Object.values(nodeMap).filter(n => n.type === 'CHART' && n.meta?.chartId);
      ...
      },
      {
        ...
        render: (
          <FilterToolbarButton
            ...
            onClearAll={() => clearAll()}
            onAddFilter={(id: string) => { onPendingFilterIds(prev => [...prev, id]); onFilterDrawerToggle(true); }}
          />
        ),
      },
      {
