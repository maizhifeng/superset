...
export default function DatasetList() {
  ...
  const {
    ...
    setPaginationModel, setDeleteTarget,
  ...
  const getDbName = (row: DatasetRow) => (row as any).database?.database_name ?? "";

  const handleRowClick = useCallback((row: DatasetRow) => {
    if (!bindMode) {
  ...
  const handleConfirmBind = async () => {
    ...
    try {
      ...
      ea.federated = { enabled: true, partner_dataset_id: b.id, databases: [dbA, dbB] };
      eb.federated = { enabled: true, partner_dataset_id: a.id, databases: [dbB, dbA] };
      await Promise.all([
  ...
  const handleUnbind = async (id: number) => {
    ...
    if (!info) return;
    try {
      const [ra, rb] = await Promise.all([getDataset(id), getDataset(info.partnerId)]);
      const ea = parseExtra((ra as any)?.extra);
      const eb = parseExtra((rb as any)?.extra);
      delete ea.federated;
      delete eb.federated;
  ...
  const columns: GridColDef[] = [
    ...
    {
      field: "schema", headerName: "模式", flex: 0.5, minWidth: 80,
      renderCell: (params) => {
    ...
    {
      field: "database", headerName: "数据库", flex: 0.7, minWidth: 100,
      valueGetter: (_v, row) => getDbName(row),
    },
    {
      field: "changed_on_delta_humanized", headerName: "最后修改", flex: 0.5, minWidth: 80,
    },
    {
      field: "kind", headerName: "类型", flex: 0.3, minWidth: 60,
      renderCell: (params) => (
    ...
    {
      field: "federated", headerName: "跨库绑定", width: 100, sortable: false,
      renderCell: (params) => {
          ...
          <Box sx={{ display: "flex", gap: 0.5 }}>
            {isBound(id) && (
              <Tooltip title="解除绑定">
                <IconButton size="small" color="error"
                  onClick={(e) => { e.stopPropagation(); handleUnbind(id); }}
                >
            ...
            <Tooltip title="编辑数据集">
              <IconButton size="small"
                onClick={(e) => { e.stopPropagation(); navigate(`/dataset/edit/${id}`); }}
              >
  ...
  return (
    <Box sx={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, p: { xs: 1.5, md: 3 }, pt: { xs: 1.5, md: 2 } }}>
      <Card variant="outlined" sx={{ borderRadius: 2, display: "flex", flexDirection: "column", flex: 1, minHeight: 0 }}>
        <CardHeader
          title={<Typography sx={{ fontSize: "0.875rem", fontWeight: 700 }}>数据集</Typography>}
          action={
            <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
              {bindMode && (
                <Typography variant="caption" color="warning.main" sx={{ fontSize: "0.75rem", fontWeight: 600 }}>
              ...
              <Button
                ...
                startIcon={<LinkIcon />}
                onClick={() => { setBindMode(!bindMode); setSelectedId(null); }}
              >
                {bindMode ? "退出绑定" : "绑定数据集"}
              </Button>
              <Button variant="contained" size="small" startIcon={<FunctionsIcon />}
        ...
        <Divider />
        <ListPageLayout
          loading={loading} error={error} hasData={rows.length > 0}
          emptyState={
            <>
              <EmptyState icon={<TableChartIcon />} title="未找到数据集"
                description={searchText ? "请调整搜索条件" : "创建第一个数据集开始构建图表"}
                action={!searchText ? (
        ...
        >
          <ResponsiveDataGrid
            ...
            paginationModel={paginationModel} rowCount={rowCount} paginationMode="server"
            ...
            renderCard={(row) => {
              ...
              return (
                <>
                  ...
                  >
                    {info && <LinkIcon color="primary" sx={{ fontSize: 12, mr: 0.25 }} />}
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.25, mt: 0.25 }}>
                    {row.schema && (
                      <Chip label={row.schema} size="small" variant="outlined"
                        sx={{ height: 16, fontSize: "0.75rem", "& .MuiChip-label": { px: 0.5 } }}
                      />
                    )}
                    <Chip label={row.kind} size="small"
                      color={row.kind === "physical" ? "primary" : "secondary"} variant="outlined"
                      sx={{ height: 16, fontSize: "0.75rem", "& .MuiChip-label": { px: 0.5 } }}
                    />
                  ...
                  {info && (
                    <Typography variant="caption" color="primary" sx={{ fontSize: "0.7rem", mt: 0.25 }}>
          ...
          <ConfirmModal
            ...
            confirmLoading={deleteLoading} danger
      ...
      <Dialog
        open={!!confirmTarget}
        onClose={() => { setConfirmTarget(null); setSelectedId(null); }}
        maxWidth="xs" fullWidth
      >
        <DialogTitle sx={{ fontSize: "0.875rem", fontWeight: 600 }}>确认跨库绑定</DialogTitle>
        <DialogContent>
          <Typography variant="body2" sx={{ mb: 2, fontSize: "0.8125rem" }}>
            将以下两个数据集关联，查询任一数据集时 SQL 会同时在两个库执行并合并结果：
          </Typography>
          <Card variant="outlined" sx={{ p: 1.5, borderRadius: 2, bgcolor: "grey.50", mb: 1 }}>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{confirmTarget?.a?.table_name}</Typography>
            <Typography variant="caption" color="text.secondary">数据库: {confirmTarget ? getDbName(confirmTarget.a) : ""}</Typography>
          </Card>
          <Typography variant="body2" sx={{ textAlign: "center", my: 0.5 }}>⟷</Typography>
          <Card variant="outlined" sx={{ p: 1.5, borderRadius: 2, bgcolor: "grey.50" }}>
            <Typography variant="body2" sx={{ fontWeight: 600 }}>{confirmTarget?.b?.table_name}</Typography>
            <Typography variant="caption" color="text.secondary">数据库: {confirmTarget ? getDbName(confirmTarget.b) : ""}</Typography>
          </Card>
        </DialogContent>
        <DialogActions>
          <Button size="small" onClick={() => { setConfirmTarget(null); setSelectedId(null); }}>取消</Button>
          <Button size="small" variant="contained" onClick={handleConfirmBind}>确认绑定</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
