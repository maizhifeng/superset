...
export default function Settings() {
  ...
  const handleAdd = () => {
    ...
    const formattedPath = newPath.startsWith('/') ? newPath.trim() : `/${newPath.trim()}`;
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 'md', mx: 'auto' }}>
      <PageHeader
        title="Settings"
        subtitle="Customize your navigation menu"
      />

      <Typography variant="subtitle2" sx={{ mb: 1.5, color: 'text.secondary', fontWeight: 600, fontSize: '0.8125rem' }}>
        Navigation Items
      </Typography>

      <Paper sx={{ borderRadius: 2, border: '1px solid', borderColor: 'divider', mb: 3 }}>
        {items.map((item, i) => (
          <Box
            key={item.id}
            sx={{
          ...
          >
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.25 }}>
              <Tooltip title="Move up">
                <span>
                  <IconButton size="small" disabled={i === 0} onClick={() => moveItem(item.id, 'up')}>
              ...
              <Tooltip title="Move down">
                <span>
                  <IconButton size="small" disabled={i === items.length - 1} onClick={() => moveItem(item.id, 'down')}>
      ...
      <Paper sx={{ p: 2, borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
        <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', flexWrap: 'wrap' }}>
          <TextField
            ...
            slotProps={{ input: { startAdornment: <Typography variant="caption" sx={{ mr: 0.5, color: 'text.disabled' }}>/</Typography> } }}
          />
          <TextField
            size="small"
            placeholder="Label"
          ...
          />
          <Button variant="contained" size="small" startIcon={<AddIcon />} onClick={handleAdd} sx={{ flexShrink: 0 }}>
            Add
          </Button>
        </Box>
        {addError && <Alert severity="error" sx={{ mt: 1, borderRadius: 1 }}>{addError}</Alert>}
      </Paper>
    </Box>
  );
}
