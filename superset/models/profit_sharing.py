from __future__ import annotations

from flask_appbuilder import Model
from sqlalchemy import Column, Integer, String, UniqueConstraint

from superset.models.papp_metadata import REGION_DOMESTIC


class ProfitSharing(Model):
    __tablename__ = "profit_sharing"
    __table_args__ = (
        UniqueConstraint(
            "papp_id",
            "channel_key",
            "region",
            name="uq_profit_sharing_papp_key_region",
        ),
        {"schema": "config"},
    )

    id = Column(Integer, primary_key=True, autoincrement=True)
    papp_id = Column(Integer, nullable=False)
    region = Column(String(32), nullable=False, default=REGION_DOMESTIC)
    # Numeric source id; NULL for oversea channels, which are keyed by name.
    channel_id = Column(Integer, nullable=True)
    channel_key = Column(String(64), nullable=False)
    papp_name = Column(String(255), nullable=True)
    channel_name = Column(String(255), nullable=True)
    上线时间 = Column(String(255), nullable=True)
    首测起始时间 = Column(String(255), nullable=True)
    二测起始时间 = Column(String(255), nullable=True)
    三测起始时间 = Column(String(255), nullable=True)
    渠道商分成 = Column(String(255), nullable=True)
    分成比例 = Column(String(255), nullable=True)
    研发分成 = Column(String(255), nullable=True)
    IP分成 = Column(String(255), nullable=True)
    分成方式 = Column(String(255), nullable=True)
    商户分成 = Column(String(255), nullable=True)
    ios虚拟支付分成 = Column(String(255), nullable=True)

    @classmethod
    def list_all(cls, region: str | None = None) -> list[ProfitSharing]:
        from superset import db

        query = db.session.query(cls)
        if region is not None:
            query = query.filter(cls.region == region)
        return query.order_by(cls.region, cls.papp_id, cls.channel_key).all()

    @classmethod
    def sync(cls, region: str | None = None) -> int:
        """Generate combos for whitelisted games and channels.

        Games only pair with channels of their own region: domestic games
        combine with domestic channels, oversea games with oversea ones.

        :param region: Restrict the sync to a single region; ``None`` syncs
            every region
        :returns: Number of combos written
        """
        from superset import db
        from superset.models.channel_metadata import ChannelMetadata
        from superset.models.papp_metadata import PappMetadata

        games = PappMetadata.list_all(region)
        channels = ChannelMetadata.list_all(region)

        whitelisted_games = [g for g in games if g.白名单控制参数 == "Y"]
        whitelisted_channels = [c for c in channels if c.白名单控制参数 == "Y"]

        channels_by_region: dict[str, list[ChannelMetadata]] = {}
        for channel in whitelisted_channels:
            channels_by_region.setdefault(channel.region, []).append(channel)

        count = 0
        for game in whitelisted_games:
            for channel in channels_by_region.get(game.region, []):
                existing = (
                    db.session.query(cls)
                    .filter(
                        cls.papp_id == game.papp_id,
                        cls.region == game.region,
                        cls.channel_key == channel.channel_key,
                    )
                    .one_or_none()
                )
                if existing:
                    existing.papp_name = game.papp_name
                    existing.channel_id = channel.channel_id
                    existing.channel_name = channel.channel_name
                    if not existing.渠道商分成:
                        existing.渠道商分成 = channel.默认分成
                    if not existing.商户分成:
                        existing.商户分成 = "1"
                    existing.ios虚拟支付分成 = channel.ios虚拟支付分成 or "0"
                else:
                    db.session.add(
                        cls(
                            papp_id=game.papp_id,
                            region=game.region,
                            channel_id=channel.channel_id,
                            channel_key=channel.channel_key,
                            papp_name=game.papp_name,
                            channel_name=channel.channel_name,
                            渠道商分成=channel.默认分成,
                            商户分成="1",
                            ios虚拟支付分成=channel.ios虚拟支付分成 or "0",
                        )
                    )
                count += 1
        db.session.commit()
        cls._compute_net_ratio()
        return count

    @classmethod
    def _compute_net_ratio(cls) -> None:
        from superset import db

        records = db.session.query(cls).all()
        for r in records:
            try:
                qd = float(r.渠道商分成 or "0")
                yf = float(r.研发分成 or "0")
                ip = float(r.IP分成 or "0")
                if r.分成方式 == "利润后分成":
                    net = (100 - qd - ip) * (100 - yf) / 100
                else:
                    net = 100 - qd - yf - ip
                r.分成比例 = f"{net:.1f}"
            except (ValueError, TypeError):
                r.分成比例 = None
        db.session.commit()
